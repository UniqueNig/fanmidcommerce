...
const ADMIN_Q = `query { orders { id status totalAmount createdAt shippingAddress { name } } }`;
const USER_Q = `query { myOrders { id status totalAmount createdAt } }`;
...
export default function NotificationsBell({ role }: { role: Role }) {
  ...
  // Admin: orders needing action. User: orders still in progress.
  const count =
    role === "admin"
      ? orders.filter((o) => ["Pending", "Processing"].includes(o.status)).length
      : orders.filter((o) => !["Delivered", "Cancelled", "Failed"].includes(o.status)).length;
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        ...
        >
          <div
            className="px-4 py-3 border-b text-[10px] tracking-[0.2em] uppercase font-bold font-['DM_Sans']"
