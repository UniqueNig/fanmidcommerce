      ...
      <div className="border divide-y" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border)" }}>
        {reviews.length === 0 ? (
          ...
          <div key={r.id} className="px-6 py-4 flex items-start justify-between gap-4">
            ...
            <button onClick={() => setDeleteId(r.id)} className="w-8 h-8 flex items-center justify-center border hover:opacity-70 flex-shrink-0" style={{ borderColor: "rgba(239,68,68,0.3)", color: "#ef4444" }}><Trash2 size={13} /></button>
          </div>
        ))}
      </div>
