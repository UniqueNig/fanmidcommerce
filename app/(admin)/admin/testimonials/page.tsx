  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setAdding(true)} className="flex items-center gap-2 px-5 py-2.5 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] hover:opacity-80" style={{ backgroundColor: "var(--accent)", color: "#000" }}><Plus size={13} /> Add</button>
      </div>
        ...
        {items.length === 0 ? (
          ...
          <div key={t.id} className="px-6 py-4 flex items-start justify-between gap-4">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <button onClick={() => update({ variables: { id: t.id, active: !t.active } })} className="text-[10px] font-bold tracking-widest uppercase font-['DM_Sans'] px-3 py-1.5 border hover:opacity-70" style={{ borderColor: t.active ? "#22c55e" : "var(--border)", color: t.active ? "#22c55e" : "var(--text-muted)" }}>{t.active ? "Shown" : "Hidden"}</button>
              <button onClick={() => setDeleteId(t.id)} className="w-8 h-8 flex items-center justify-center border hover:opacity-70" style={{ borderColor: "rgba(239,68,68,0.3)", color: "#ef4444" }}><Trash2 size={13} /></button>
            </div>
          </div>
        ))}
      </div>
