    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className="border p-5" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border)" }}>
            ...
            {filtered.map((p) => {
              ...
              return (
                <div key={p.id} className="px-6 py-3 flex items-center gap-4">
                  <div className="w-10 h-12 flex-shrink-0 overflow-hidden" style={{ backgroundColor: "var(--bg-secondary)" }}>
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-20 px-2 py-1.5 text-sm font-['DM_Sans'] outline-none border text-center"
                      style={{ backgroundColor: "var(--bg-primary)", borderColor: "var(--border)", color: "var(--text-primary)" }}
                    />
                    <button
                      onClick={() => save(p.id)}
                      disabled={!changed || savingId === p.id}
                      className="w-9 h-9 flex items-center justify-center disabled:opacity-30 transition-opacity"
                      style={{ backgroundColor: savedId === p.id ? "#22c55e" : "var(--accent)", color: "#000" }}
                      title="Save stock"
                    >
