  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        ...
        {subs.length > 0 && (
          <button onClick={exportCsv} className="flex items-center gap-2 px-5 py-2.5 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] border hover:opacity-70" style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}>
      ...
      <div className="border divide-y" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border)" }}>
        {subs.length === 0 ? (
          ...
          <div key={s.id} className="px-6 py-4 flex items-center justify-between">
            ...
            <button onClick={() => setDeleteId(s.id)} className="w-8 h-8 flex items-center justify-center border hover:opacity-70" style={{ borderColor: "rgba(239,68,68,0.3)", color: "#ef4444" }}><Trash2 size={13} /></button>
          </div>
        ))}
      </div>
