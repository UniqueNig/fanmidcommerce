...
const inputClass = "w-full px-3 py-2 text-sm font-['DM_Sans'] outline-none border";
...
export default function AdminTeamPage() {
  ...
  if (error)
    return <div className="flex flex-col items-center justify-center py-32 gap-4"><p className="text-sm font-['DM_Sans']" style={{ color: "#ef4444" }}>{error.message}</p><button onClick={() => refetch()} className="flex items-center gap-2 px-5 py-2.5 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] border" style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}><RefreshCw size={13} /> Retry</button></div>;
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 px-5 py-2.5 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] hover:opacity-80" style={{ backgroundColor: "var(--accent)", color: "#000" }}><Plus size={13} /> Add Member</button>
      </div>
      ...
      {showForm && (
        <div className="border p-5 space-y-4" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--accent)" }}>
          ...
          <div className="flex gap-4 items-start">
            <div className="w-20 h-24 flex-shrink-0 overflow-hidden border" style={{ borderColor: "var(--border)", backgroundColor: "var(--bg-secondary)" }}>
          ...
          <div className="flex gap-2">
            <button onClick={save} disabled={creating || updating || uploading} className="flex items-center gap-2 px-5 py-2 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] hover:opacity-80 disabled:opacity-50" style={{ backgroundColor: "var(--accent)", color: "#000" }}>{creating || updating ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />} Save</button>
            <button onClick={() => { setShowForm(false); setForm(EMPTY); setEditId(null); }} className="flex items-center gap-2 px-5 py-2 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] border" style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}><X size={12} /> Cancel</button>
          </div>
        </div>
      )}
        ...
        {members.length === 0 ? (
          ...
          <div key={m.id} className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="w-12 h-14 overflow-hidden flex-shrink-0" style={{ backgroundColor: "var(--bg-secondary)" }}>
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => openEdit(m)} className="w-8 h-8 flex items-center justify-center border hover:opacity-70" style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}><Edit2 size={13} /></button>
              <button onClick={() => setDeleteId(m.id)} className="w-8 h-8 flex items-center justify-center border hover:opacity-70" style={{ borderColor: "rgba(239,68,68,0.3)", color: "#ef4444" }}><Trash2 size={13} /></button>
            </div>
          </div>
        ))}
      </div>
