  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => setAdding(true)} className="flex items-center gap-2 px-5 py-2.5 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] hover:opacity-80" style={{ backgroundColor: "var(--accent)", color: "#000" }}><Plus size={13} /> Add Method</button>
      </div>
      ...
      {adding && (
        <div className="border p-5 space-y-4" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--accent)" }}>
          ...
          <div className="flex gap-2">
            <button onClick={add} disabled={creating} className="flex items-center gap-2 px-5 py-2 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] hover:opacity-80 disabled:opacity-50" style={{ backgroundColor: "var(--accent)", color: "#000" }}>{creating ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />} Save</button>
            <button onClick={() => setAdding(false)} className="flex items-center gap-2 px-5 py-2 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] border" style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}><X size={12} /> Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="border divide-y" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border)" }}>
        {methods.length === 0 ? (
          ...
          <div key={m.id} className="px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => update({ variables: { id: m.id, active: !m.active } })} className="text-[10px] font-bold tracking-widest uppercase font-['DM_Sans'] px-3 py-1.5 border hover:opacity-70" style={{ borderColor: m.active ? "#22c55e" : "var(--border)", color: m.active ? "#22c55e" : "var(--text-muted)" }}>{m.active ? "Active" : "Inactive"}</button>
