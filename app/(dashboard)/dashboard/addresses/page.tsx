  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between">
        ...
        <button onClick={openAdd} className="flex items-center gap-2 px-5 py-2.5 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] hover:opacity-80" style={{ backgroundColor: "var(--accent)", color: "#000" }}>
      ...
      {showForm && (
        <div className="border p-5 space-y-4" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--accent)" }}>
          ...
          <div className="flex gap-2">
            <button onClick={save} disabled={adding || updating} className="flex items-center gap-2 px-5 py-2 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] hover:opacity-80 disabled:opacity-50" style={{ backgroundColor: "var(--accent)", color: "#000" }}>
              {adding || updating ? <Loader2 size={12} className="animate-spin" /> : <Check size={12} />} Save
            </button>
            <button onClick={() => { setShowForm(false); setForm(EMPTY); setEditId(null); }} className="flex items-center gap-2 px-5 py-2 text-xs font-bold tracking-widest uppercase font-['DM_Sans'] border" style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}>
              <X size={12} /> Cancel
            </button>
          </div>
        </div>
      ...
      {loading && addresses.length === 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 gap-2 border" style={{ backgroundColor: "var(--card-bg)", borderColor: "var(--border)" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {addresses.map((a) => (
            <div key={a.id} className="border p-5" style={{ backgroundColor: "var(--card-bg)", borderColor: a.isDefault ? "var(--accent)" : "var(--border)" }}>
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <MapPin size={14} style={{ color: "var(--accent)" }} />
                  <span className="text-sm font-bold font-['DM_Sans']" style={{ color: "var(--text-primary)" }}>{a.label || "Address"}</span>
                  {a.isDefault && <span className="text-[9px] font-bold tracking-widest uppercase px-2 py-0.5 font-['DM_Sans']" style={{ backgroundColor: "color-mix(in srgb, var(--accent) 15%, transparent)", color: "var(--accent)" }}>Default</span>}
                </div>
              </div>
